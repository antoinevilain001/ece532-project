        ...
        if (!resetn) begin
            ...
            if (update_game) begin
                ...
                if (ball_x <= 0 + GAME_BORDER) begin
                    ...
                    && (ball_y < paddle1_y + paddle1_height)) begin
                        ball_xspeed <= ball_xspeed * -1;
                        ball_x <= ball_x - ball_xspeed; // get away from paddle
                        // yspeed: which part of the paddle?
                        if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*1/8) begin
                            ball_yspeed <= -4;
                            ball_y <= ball_y - 4;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*2/8) begin
                            ball_yspeed <= -3;
                            ball_y <= ball_y - 3;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*3/8) begin
                            ball_yspeed <= -2;
                            ball_y <= ball_y - 2;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*4/8) begin
                            ball_yspeed <= -1;
                            ball_y <= ball_y - 1;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*5/8) begin
                            ball_yspeed <= 1;
                            ball_y <= ball_y + 1;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*6/8) begin
                            ball_yspeed <= 2;
                            ball_y <= ball_y + 2;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*7/8) begin
                            ball_yspeed <= 3;
                            ball_y <= ball_y + 3;
                        end
                        else if (ball_y < paddle1_y - BALL_SIZE + (paddle1_height + BALL_SIZE)*8/8) begin
                            ball_yspeed <= 4;
                            ball_y <= ball_y + 4;
                        end
                    end
                    ...
                    && (ball_y < paddle2_y + paddle2_height)) begin
                        ball_xspeed <= ball_xspeed * -1;
                        ball_x <= ball_x - ball_xspeed; // get away from paddle
                        // yspeed: which part of the paddle?
                        if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*1/8) begin
                            ball_yspeed <= -4;
                            ball_y <= ball_y - 4;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*2/8) begin
                            ball_yspeed <= -3;
                            ball_y <= ball_y - 3;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*3/8) begin
                            ball_yspeed <= -2;
                            ball_y <= ball_y - 2;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*4/8) begin
                            ball_yspeed <= -1;
                            ball_y <= ball_y - 1;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*5/8) begin
                            ball_yspeed <= 1;
                            ball_y <= ball_y + 1;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*6/8) begin
                            ball_yspeed <= 2;
                            ball_y <= ball_y + 2;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*7/8) begin
                            ball_yspeed <= 3;
                            ball_y <= ball_y + 3;
                        end
                        else if (ball_y < paddle2_y - BALL_SIZE + (paddle2_height + BALL_SIZE)*8/8) begin
                            ball_yspeed <= 4;
                            ball_y <= ball_y + 4;
                        end
                    end
        ...
        if (!resetn) begin
            ...
            if (update_game) begin
                ...
                if (user_dir == 2'b01) begin // user wants to move down
                    // move if able
                    if (paddle1_y + paddle1_height < GAME_HEIGHT - 1) begin
                        paddle1_y <= paddle1_y + paddle1_speed;
                    end
                end
                ...
                if (user_dir2 == 2'b01) begin // user wants to move down
                    // move if able
                    if (paddle2_y + paddle2_height < GAME_HEIGHT - 1) begin
                        paddle2_y <= paddle2_y + paddle2_speed;
                    end
                end
